...
use core::GameEndStatus;
use std::path::PathBuf;
use std::io::Write;
use clap::{Parser, Subcommand, ValueEnum};
use rand::Rng;

use crate::{core::{run_game, PlayerMark}, game::connect_four::C4Board, player::random::RandomAi};
mod core;
mod player;
mod game;
...
fn record_result(outfile: &PathBuf, player1: PlayerSpec, player2: PlayerSpec, result: GameEndStatus) -> Result<(), std::io::Error>{
...
fn run_c4(player1: PlayerSpec, player2: PlayerSpec) -> GameEndStatus {
    println!("Running a game of connect4 with players {:?} and {:?}", player1, player2);
